...
public class DvdBean {
	...
	public void save() {
		GeneroRepository generoRepository = new GeneroRepository(this.getManager());
	...
	/*
	 * public List<DVD> dvds() { List<DVD> dvds = this.dvds;
	 * 
	 * try { // DvdRepository repository = new //
	 * DvdRepository(this.getManager().createNamedQuery("DVD.findAll")); dvds =
	 * ((DvdBean) em.createNamedQuery("DVD.findAll")).dvds(); // dvds =
	 * repository.getDVDs(); } catch (Exception e) { e.printStackTrace();
	 * System.out.println(dvds); } return dvds; }
	 */
